-- Little Rock termite territory map: per-zip metrics.
-- Run via RevHawk run_query. Save the returned rows to pipeline/littlerock/rows.json, then run build.mjs.
-- Scope: Little Rock office (1), Arkansas zips in the valid AR range (71600-72999).
-- This office covers a much wider area than a single metro -- Little Rock/North Little Rock,
-- Conway, Cabot, Benton/Bryant, Hot Springs, Searcy, Pine Bluff, Russellville, Heber Springs,
-- and a long tail of small towns around them. No county whitelist is used (unlike St. Louis) --
-- there are too many legitimate counties here to enumerate safely, so this relies on
-- build.mjs's existing MIN_ACCOUNTS(5) filter to drop noise instead.
-- The termite program has not launched here yet -- active_termite / termite_arv will be ~0.
...
WITH cust AS (
  ...
  WHERE fieldRoutes_officeID = '1'
    AND fieldRoutes_state = 'AR'
    AND fieldRoutes_zip BETWEEN '71600' AND '72999'
),
zip_meta AS (
...
-- kept identical to the Wichita/St. Louis queries so all three stay comparable.
term AS (
